...
async def _fetch_bars(symbols: List[str]) -> Dict[str, pd.DataFrame]:
    """Fetch last 6 daily bars for symbols — Massive primary, Alpaca fallback."""
    from data.massive_client import massive_client
    from trading.alpaca_client import alpaca_client

    try:
        result = await massive_client.get_bars_multi(symbols, days=6)
        if result and any(v is not None and not v.empty for v in result.values()):
            return result
    except Exception as e:
        logger.debug(f"sector_analysis: Massive fetch failed: {e}")

    try:
        return await alpaca_client.get_bars_multi(symbols, limit=6)
    except Exception as e:
        logger.warning(f"sector_analysis: Alpaca fetch also failed: {e}")
        return {}
...
async def get_sector_performance() -> Dict:
    """
    Fetch 1d and 5d returns for sector ETFs and broad market indices.
...

        {
            ...
            "sectors": {
                "Technology": {
                    "etf":      "XLK",
                    "pct_1d":   1.2,
                    "pct_5d":   2.3,         # None if < 6 bars available
                    "vs_spy_1d": 0.8,        # XLK - SPY
                    "trend":    "leading",   # "leading" | "neutral" | "lagging"
                },
                ...
            },
            "broad": {
                "SPY": {"pct_1d": 0.4, "pct_5d": 1.1},
                "QQQ": {"pct_1d": 0.9, "pct_5d": 2.2},
                "IWM": {"pct_1d": -0.2, "pct_5d": 0.1},
            }
        }
    """
...
def format_sector_summary(sector_perf: Dict) -> str:
    ...
    Example::
        ...
        Market: SPY +0.4% | QQQ +0.9% | IWM -0.2%
    ...
    for etf in ["SPY", "QQQ", "IWM"]:
        ...
        p = d.get("pct_1d")
        if p is not None:
            mkt_parts.append(f"{etf} {'+' if p >= 0 else ''}{p:.1f}%")
    if mkt_parts:
        lines.append("Market: " + " | ".join(mkt_parts))
